...
function check_if_in_view() {
	...
	$.each($animation_elements_in, function() {
		...
		if ((element_bottom_position >= window_top_position+100) &&
			...
			$element.addClass('animated fadeInUp').removeClass('animate-hide');
		}
	});
	...
	$.each($animation_elements_left, function() {
		...
		if ((element_bottom_position >= window_top_position+100) &&
			(element_top_position <= window_bottom_position)) {
			$element.addClass('animated fadeInLeft').removeClass('animate-hide');
		}
	});
	...
	$.each($animation_elements_right, function() {
		...
		if ((element_bottom_position >= window_top_position+100) &&
			(element_top_position <= window_bottom_position)) {
			$element.addClass('animated fadeInRight').removeClass('animate-hide');
		}
	});
	$.each($animation_elements_zoom, function() {
		...
		if ((element_bottom_position >= window_top_position+100) &&
			(element_top_position <= window_bottom_position)) {
			$element.addClass('animated zoomIn').removeClass('animate-hide');
		}
	});
}
